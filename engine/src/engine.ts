...
export function createEngine(store: Store) {
  function getNextBestAskPrice(symbol: string, startFrom: number = -1) {
    const orderbook = store.orderbooks.get(symbol);
  ...
  function handleEngineRequest(message: EngineRequest): unknown {
     ...
     * 3. Call order book / balance / order logic.
    ...
    if (message.type === "init_user_balance") {
      ...
      store.balances.set(userId as string, initialUserBalances);
      return;
    }
    if (message.type === "create_order") {
      ...
      // seed if user non-exstent on the store.balances
      let userBalance = store.balances.get(userId);
      if (!userBalance) {
        ...
        if (side === "buy") {
          ...
          while (remainingQty > 0 && bestAskPrice && bestAskPrice <= price) {
            ...
            for (let i = ordersAtPrice.length - 1; i >= 0; i--) {
              ...
              if (availableQty > remainingQty) {
                ...
                let currentOrder = store.orders.get(currentOrderId);
                if (!currentOrder) {
                  currentOrder = {
                ...
                const sellerUserId = store.orders.get(
                  restingOrder.orderId,
                )?.userId!;
                const sellerBalance = store.balances.get(sellerUserId)!;
          ...
          if (remainingQty > 0) {
            ...
            if (filledQty === 0) {
              ...
              store.orders.set(currentOrderId, {
                ...currentOrder,
                fills: [],
              });
            }
            ...
            userBalance[PRIMARY_CURRENCY] = {
              ...
              locked:
                userBalance[PRIMARY_CURRENCY].locked + remainingTotalPrice,
            };
          }
        } else if (side === "sell") {
          ...
          while (remainingQty > 0 && bestBidPrice && bestBidPrice >= price) {
            ...
            for (let i = ordersAtPrice.length - 1; i >= 0; i--) {
              ...
              if (availableQty > remainingQty) {
                ...
                const buyerUserId = store.orders.get(
                  restingOrder.orderId,
                )?.userId!;
                const buyerBalance = store.balances.get(buyerUserId)!;
      ...
      const averagePrice = +(totalCost / currentOrder.filledQty).toFixed(2);
      return {
        orderId: currentOrderId,
    ...
    if (message.type === "get_user_balance") {
      ...
      const userBalance = store.balances.get(userId);
      if (!userBalance) {
    ...
    if (message.type === "cancel_order") {
      ...
      const cancellableOrder = store.orders.get(orderId);
      ...
      const orderbook = store.orderbooks.get(cancellableOrder.symbol);
      if (!orderbook) {
      ...
      const userBalance = store.balances.get(cancellableOrder.userId)!;
